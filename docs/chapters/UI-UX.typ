...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [#strong[World Map];], [Full-screen Leaflet.js; primary interface;
    all overlays accessible here],
    [#strong[City Panel];], [Slide-in overlay; buildings, resources,
    queue, population],
    [#strong[Military Panel];], [Army management, march orders, garrison
    assignment],
    [#strong[Research Tree];], [Visual era-based tech tree],
    [#strong[Divine Panel];], [God selection, Favor meter, available
    powers],
    [#strong[Diplomacy Hub];], [Relations matrix, active treaties,
    diplomatic messages],
    [#strong[Alliance Portal];], [Alliance management, forum, bank, war
    status],
    [#strong[Event Feed];], [Chronological personal notification
    stream],
    [#strong[Leaderboard];], [Prestige, active victory condition
    progress],
    [#strong[World Map Overlays];], [Territory, resources, armies (own +
    scouted), active trade dispatches],
  )]
  , kind: table
  )

=== 19.3 Responsive Design
...
#figure(
  align(center)[#table(
    columns: 2,
    align: (auto,auto,),
    table.header([Element], [Target],),
    table.hline(),
    [Initial page load], [\< 3s on 3G],
    [Map tile render], [\< 100ms per zoom],
    [City panel open], [\< 50ms],
    [WebSocket latency], [\< 200ms],
    [Production update (visible)], [Smooth, every 5s visual refresh],
  )]
  , kind: table
  )
