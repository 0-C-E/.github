...
#figure(
  align(center)[#table(
    ...
    table.hline(),             [#strong[Continental];], [Large landmasses with inland
    seas],
    [Optional coastal],        [#strong[Archipelago];], [Island chains, heavy ocean
    presence],
    [Full naval layer active], [#strong[Pangaea];],     [Single giant continent,
    land-locked
    majority],
    [Minimal naval],           [#strong[Islands];],     [Medium islands separated
    by sea],
    [Full naval
    layer active],
  )],
...
#figure(
  align(center)[#table(
    ...
    research boost or resource cache],              [#strong[Sacred Site];],
...
#figure(
  align(center)[#table(
    ...
    table.hline(),                                         [#strong[Passive --- Cradle
    of Civilization];], [All building
    ...
    knowledge],                                                      [#strong[Passive ---
    Irrigation Mastery];],     [Farm buildings produce
    20% more food.],
    [Sumerian civilization was built on canal
    irrigation of the Tigris-Euphrates],                                                      [#strong[Active ---
    Cuneiform Decree] (cooldown 48h/world
    speed)],                                               [Instantly complete the
    current lowest-tier building in the
    construction queue.],
    [Writing enabled administrative efficiency and
    record-keeping],                                                      [#strong[Unique
    Unit --- Sumerian Spearman] (replaces Spearmen
    T1)],                                               [Standard stats + formation
    bonus: for every 10% of the army
    composed of Spearmen, Spearmen gain +3% defense (max
    \+15%).],
    [Earliest recorded phalanx-style infantry formations], [#strong[Unique Building ---
    Ziggurat] (replaces
    Temple)],                                               [Functions as Temple
    ...
    and scholarly centers],                                                      [#strong[God
    Affinity];],                       [Anunnaki pantheon. Bonus Favor from
...
#figure(
  align(center)[#table(
    ...
    align: (auto, auto, auto),
    table.header([Attribute], [Design], [Historical Basis]),
    table.hline(),                                   [#strong[Passive --- Thalassocracy];],      [Naval
    units move 25% faster.
    Gold bonus from completed trades +20% when sent from coastal
    cities.],
    [Greek naval dominance; Athens\' trade empire],  [#strong[Passive --- City-State
    Network];], [Each additional city
    ...
    plateaus.],
    [Greek polis network; economic interdependence], [#strong[Active --- Agora] (cooldown
    36h)], [For 8 hours, gold
    bonuses from all completed trades doubled.],
    [The Agora was the
    commercial and civic heart of every Greek city],                                                [#strong[Unique
    Unit --- Trireme] (naval unit, replaces standard
    warship)],                                           [Faster than standard naval
    units; rams deal bonus damage
    on first contact; can transport a small number of land units.],
    [The
    trireme was the defining Greek naval weapon],                                                [#strong[Unique
    Unit --- Hoplite] (replaces Heavy
    Infantry)],                                           [Standard stats + Phalanx
    ...
    relied on collective formation discipline],                                                [#strong[Unique
    Building --- Stoa] (replaces Marketplace
    T2)],                                           [Generates gold + increases simultaneous
    trade dispatch
    capacity by 1 + grants 10 bonus Culture Points per completed
    trade.],
    [The Stoa was a covered walkway serving as marketplace and
    philosophical gathering place],                                                [#strong[God
    Affinity];],                   [Full Olympian pantheon --- Greece has
...
#figure(
  align(center)[#table(
    ...
    align: (auto, auto, auto),
    table.header([Attribute], [Design], [Historical Basis]),
    table.hline(), [#strong[Passive --- Seat of the Pharaoh];],                 [One
    city per player is
    ...
    moving the capital was a rare, politically seismic event],              [#strong[Passive ---
    Inundation Cycle];],                    [The Capital receives a
    periodic food surge every 72 hours (representing the Nile flood
    cycle).],
    [The annual Nile inundation deposited fertile silt, making
    Egypt the breadbasket of the ancient world],              [#strong[Active ---
    Pharaoh\'s Decree] (cooldown 72h)],      [Apply 7
    ...
    apparatus],              [#strong[Unique Unit --- Medjay] (replaces Light Infantry)], [Elite
    ...
    who became Egypt\'s elite police and royal guard],              [#strong[Unique
    Unit --- War Chariot] (replaces Cavalry)],   [Faster
    than standard cavalry; +20% attack on open terrain (plains, desert);
    −20% on hills or forest.],
    [Egyptian chariotry was among the
    earliest and most effective in the ancient world],              [#strong[Unique
    ...
    religious institution in ancient Egypt],              [#strong[Capital Capture
    Rule];],                            [If the Capital is captured, the
    ...
    Rome) was existential],              [#strong[God Affinity];],                                    [Egyptian
...
#figure(
  align(center)[#table(
    ...
    align: (auto, auto, auto),
    table.header([Attribute], [Design], [Historical Basis]),
    table.hline(), [#strong[Passive --- Provincial Administration];],           [Each
    city Rome
    ...
    [Roman provincial
    administration made empire economically viable],              [#strong[Passive ---
    Roman Engineering];],                   [Construction speed +10%
    per city currently owned (stacks). A 3-city Rome builds 30% faster
    than a 1-city Rome.],
    [Roman engineers built faster and more durably
    than any contemporary civilization],              [#strong[Active --- Senatus
    Consultum] (cooldown
    96h)],                                                            [Redistribute
    resources freely between all owned cities
    instantly (no travel time, no loss).],
    [The Roman Senate coordinated
    resource allocation across the entire empire],              [#strong[Unique Unit ---
    ...
    durable heavy infantry of antiquity],              [#strong[Unique Unit --- Ballista]
    (replaces Siege Weapon)], [Longer
    ...
    deployed both offensively and as wall-mounted defensive weapons],              [#strong[Unique
    ...
    could support],              [#strong[Unique Building --- Amphitheatre] (replaces
    Theatre)],                                                            [Generates
    ...
    religious, and social functions simultaneously],              [#strong[God Affinity];],                                    [Roman
...
#figure(
  align(center)[#table(
    ...
    align: (auto, auto, auto),
    table.header([Attribute], [Design], [Historical Basis]),
    table.hline(), [#strong[Passive --- Jungle Adaptation];], [Maya cities founded
    in
    ...
    civilizations],              [#strong[Passive --- Sacred Calendar];],   [Every
    13 in-game days (a
    ...
    cycles],              [#strong[Active --- Blood Offering] (no cooldown, resource
    cost)],                                          [Sacrifice population or military
    ...
    mechanism for communicating with Maya gods],              [#strong[Active ---
    Ritual Warfare] (army order, replaces standard
    Raid)],                                          [Instead of looting resources
    ...
    conquest],              [#strong[Unique Unit --- Eagle Warrior] (replaces Light
    Infantry)],                                          [Fast, high attack, low
    ...
    orders tied to religious ceremony],              [#strong[Unique Unit --- Atlatl
    Skirmisher] (replaces
    Archer)],                                          [Longer range than standard
    ...
    in jungle terrain],              [#strong[Unique Building --- Temple Pyramid]
    (replaces Grand Temple
    T3)],                                          [Massively increased Favor generation.
    ...
    sacrificial altars and divine residences],              [#strong[Unique Building ---
    Observatory] (replaces Academy
    T2)],                                          [Generates research AND tracks
    ...
    tracked celestial cycles with extraordinary precision],              [#strong[God
    Affinity];],                  [Maya pantheon (Itzamná, Kukulkan,
