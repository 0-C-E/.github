...
==== 15.1.1 Ownership Model --- King of the Hill
<1511-ownership-model--king-of-the-hill>
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [#strong[Resource Tribute];], [Village pays a resource tribute every
    production cycle (type varies by village --- farms pay food,
    quarries pay stone, etc.)],
    [#strong[Exclusive Recruitment];], [Owner can recruit a small number
    of village-specific units for gold (unit type varies by village
    culture)],
    [#strong[Trade Monopoly];], [Owner receives +25% gold on any trade
    dispatched through that village\'s region],
  )]
  , kind: table
  )

==== 15.1.3 Non-Owner Interactions
<1513-non-owner-interactions>
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [#strong[Trade];], [Any player], [Send a merchant to trade --- gold
    exchange only, no resource tribute, no recruitment access],
    [#strong[Raid];], [Any player], [One-time attack on the village
    treasury --- yields a resource cache but does not change ownership
    unless garrison is defeated],
    [#strong[Seize];], [Any player], [Defeat garrison → become new
    owner],
  )]
  , kind: table
  )

==== 15.1.4 Village Garrison
<1514-village-garrison>
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [Early (0--30 days)], [Weak --- defeatable by a small raiding
    party],
    [Mid (30--90 days)], [Moderate --- requires a real army commitment],
    [Late (90+ days)], [Strong --- discourages casual seizure attempts],
  )]
  , kind: table
  )
...
#figure(
  align(center)[#table(
    ...
    table.hline(),
    [#strong[Farming Settlement];], [Food], [Militia Infantry (cheap
    light infantry)],
    [#strong[Stone Quarry];], [Stone], [Siege Laborers (boost siege
    weapon construction speed)],
    [#strong[Hunting Ground];], [Wood], [Scout Cavalry (fast, low combat
    strength, high map reveal range)],
    [#strong[River Port];], [Gold], [River Traders (boost trade dispatch
    gold yield)],
    [#strong[Forge Village];], [Metal], [Auxiliary Warriors (medium
